...
class CupacReporter(Reporter):
            ...
            resume_data.append(
                {
                    ...
                    "variance_reduction_cv": f"{vr_cv:.1f}%"
                    if vr_cv is not None
                    else "-",
                    "variance_reduction_real": f"{vr_real:.1f}%"
                    if vr_real is not None
                    else "-",
                }
            )
    ...
    def _get_group_means(
        experiment_data: ExperimentData, target_name: str
    ) -> dict[str, float | None]:
        ...
        group_diff_ids = experiment_data.get_ids(
            GroupDifference, searched_space=ExperimentDataEnum.analysis_tables
        ...
        for gd_id in group_diff_ids:
            ...
            if gd_target == target_name:
                ...
                control_mean = (
                    gd_data.data["control mean"].iloc[0]
                    if "control mean" in gd_data.columns
                    else None
                )
                test_mean = (
                    gd_data.data["test mean"].iloc[0]
                    if "test mean" in gd_data.columns
                    else None
                )
                return {"control_mean": control_mean, "test_mean": test_mean}

        return {"control_mean": None, "test_mean": None}

    @staticmethod
    def extract_variance_reductions(experiment_data: ExperimentData) -> Dataset | None:
        ...
        for key in cupac_report_keys:
            ...
            original_means = CupacReporter._get_group_means(
                experiment_data, target_name
            )
            cupac_means = CupacReporter._get_group_means(
                experiment_data, f"{target_name}_cupac"
            )

            if (
                original_means["control_mean"] is not None
                and cupac_means["control_mean"] is not None
            ):
                control_mean_bias = (
                    original_means["control_mean"] - cupac_means["control_mean"]
                )

            if (
                original_means["test_mean"] is not None
                and cupac_means["test_mean"] is not None
            ):
