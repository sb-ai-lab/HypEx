...
class DatasetBase:
    ...
    def _select_backend_from_data(
        data: Any,
        session: spark.SparkSession = None,
        ...
        raise TypeError(
            "data must be an instance of either"
    ...
    def _set_all_roles(self, roles: dict[str, ABCRole]) -> dict[str, ABCRole]:
    ...
    def __init__(
        ...
        if backend is not None:
            ...
            self._backend_data = self._select_backend_from_str(
                data, backend, data_compression, session, non_compresion_cols
            )
        ...
        if roles is None and data.hasattr("roles") and data.roles is not None:
            ...
            if data is not None and hasattr(self._backend_data, "columns"):
                invalid_columns = [
                    i for i in roles.keys() if i not in self._backend_data.columns
                ]
                if invalid_columns:
    ...
    def reset_index(self, drop: bool = False, **kwargs) -> Self:
    ...
    def __binary_magic_operator(self, other: Any, func_name: str) -> Any:
        ...
        self_raw = (
            ...
            if hasattr(self.backend_data, "data")
            else self.backend_data
        )

        if isinstance(other, self.__class__):
            if type(other._backend_data) is not type(self._backend_data):
                raise BackendTypeError(
        ...
        for col in actual_columns:
            new_roles[col] = deepcopy(
                self.roles.get(col, self.default_role or DefaultRole())
            )
    ...
    def __pos__(self) -> Self:
        return self.__class__(
            roles=self.roles, data=(+self._backend_data), session=self.session
        )
    ...
    def __abs__(self) -> Self:
        return self.__class__(
            roles=self.roles, data=abs(self._backend_data), session=self.session
        )
    ...
    def __round__(self, ndigits: int = 0) -> Self:
        return self.__class__(
            roles=self.roles,
            data=round(self._backend_data, ndigits),
    ...
    def _convert_data_after_agg(self, result: Any) -> Self | ScalarType | None:
        if result is None:
            return None
    ...
    def map(self, func: Callable[..., Any], na_action: Any = None, **kwargs) -> Self:
        return self.__class__(
            roles=self.roles,
    ...
    def mode(self, numeric_only: bool = False, dropna: bool = True) -> Self:
    ...
    def agg(self, func: str | list) -> Self | ScalarType | None:
        return self._convert_data_after_agg(self._backend_data.agg(func))

    def std(self, skipna: bool = True, ddof: int = 1) -> Self | ScalarType | None:
        return self._convert_data_after_agg(
            self._backend_data.std(skipna=skipna, ddof=ddof)
        )

    def quantile(self, q: float = 0.5) -> Self | ScalarType | None:
    ...
    def coefficient_of_variation(self) -> Self | ScalarType | None:
        return self._convert_data_after_agg(
            self._backend_data.coefficient_of_variation()
        )
    ...
    def value_counts(
        ...
        new_columns = (
            t_data.columns if hasattr(t_data, "columns") else list(t_data.data)
        )
    ...
    def replace(
        ...
        return self.__class__(
            self.roles,
            data=self._backend_data.replace(
