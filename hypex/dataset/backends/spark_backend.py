...
class SparkNavigation(DatasetBackendNavigation):
    ...
    def _wrap_result(
        self, result: ps.DataFrame | ps.Series | Any, wrap_series: bool = False
    ) -> "SparkNavigation" | ps.Series | Any:
    ...
    def __init__(
        self,
        data: (
            ps.DataFrame | SparkDF | pd.DataFrame | dict[str, Any] | str | None
        ) = None,
...
class SparkDataset(SparkNavigation, DatasetBackendCalc):
    ...
    def dot(
        self, other: "SparkDataset" | np.ndarray | pd.DataFrame
    ) -> "SparkDataset" | float:
