...
#         left_cols = self._public_columns
#         right_cols = right._public_columns
        
#         if left_index:
#             left_on = UTILITY_INDEX_COL_NAME
#         if right_index:
#             right_on = UTILITY_INDEX_COL_NAME
...
#         if left_on:
#             for col in left_on:
#                 if col not in left_cols and col != UTILITY_INDEX_COL_NAME and col != UTILITY_PHYSICAL_INDEX_COL_NAME:
#                     raise MergeOnError(f"Column '{col}' not found in left dataset. Available: {left_cols}")
        
#         if right_on:
#             for col in right_on:
#                 if col not in right_cols and col != UTILITY_INDEX_COL_NAME and col != UTILITY_PHYSICAL_INDEX_COL_NAME:
#                     raise MergeOnError(f"Column '{col}' not found in right dataset. Available: {right_cols}")
        
#         if len(left_on) == 1 and len(right_on) == 1:
#             join_condition = left_df[left_on[0]] == right_df[right_on[0]]
#         else:
#             join_condition = [
#                 left_df[l] == right_df[r] 
#                 for l, r in zip(left_on, right_on)
#             ]
        
#         with self._cached(left_df):
#             with self._cached(right_df):
#                 joined = left_df.join(right_df, join_condition, how=how)
                
#                 right_renames = {}
#                 for col in right_df.columns:
#                     if col in (UTILITY_INDEX_COL_NAME, UTILITY_PHYSICAL_INDEX_COL_NAME):
#                         continue
...
#                 if left_on == right_on:
#                     for col in right_on:
#                         if col != UTILITY_INDEX_COL_NAME:
#                             duplicate_col = f"{col}{suffixes[1]}"
#                             if duplicate_col in result.columns:
#                                 result = result.drop(duplicate_col)
                
#                 utility_cols_to_drop = []
#                 for col in result.columns:
#                     if col.startswith("__") and col != UTILITY_INDEX_COL_NAME:
#                         utility_cols_to_drop.append(col)
#                 for col in utility_cols_to_drop:
...
#         labels_list = Adapter.to_list(labels) if labels is not None else []
...
#             ]
#             removed_labels = set(labels_list) - set(safe_labels)
...
#             existing_cols = set(self._public_columns)
#             if errors == "raise":
#                 missing = set(safe_labels) - existing_cols
#                 if missing:
#                     raise KeyError(f"Columns not found: {missing}")
            
#             cols_to_drop = [l for l in safe_labels if l in existing_cols]
            
#             if not cols_to_drop:
#                 return self
            
#             result_df = self.data.drop(*cols_to_drop)
#             return self.__class__(
#                 data=result_df,
#                 session=self.session,
#                 physical_index_actual_flag=self._physical_index_actual_flag
...
#             existing_index_values = set(self.index)
            
#             if errors == "raise":
#                 missing = set(labels_list) - existing_index_values
#                 if missing:
#                     raise KeyError(f"Index values not found: {missing}")
            
#             indices_to_drop = [l for l in labels_list if l in existing_index_values]
            
#             if not indices_to_drop:
#                 return self
            
#             result_df = self.data.filter(
#                 ~F.col(UTILITY_INDEX_COL_NAME).isin(indices_to_drop)
#             )
...
#         if params_sum > 1:
#             raise ValueError(
#                 "Cannot specify both 'items', 'regex', and 'like' simultaneously. "
#                 "Use only one filtering method."
#             )
        
#         if axis == 1:
#             if items is None and regex is None and like is None:
#                 return self
            
#             if items is not None:
#                 valid_items = [col for col in items if col in self._public_columns]
#                 if not valid_items:
#                     return self.__class__(
...
#                         physical_index_actual_flag=False
#                     )
#                 result_df = self.data.select(*[F.col(c) for c in valid_items])
            
#             elif regex is not None:
#                 matched_cols = [
#                     col for col in self._public_columns 
...
#                         physical_index_actual_flag=False
#                     )
#                 result_df = self.data.select(*[F.col(c) for c in matched_cols])
...
#             if items is not None:
#                 existing_index_values = set(self.index)
#                 valid_items = [idx for idx in items if idx in existing_index_values]
...
#                 result_df = self.data.filter(
#                     F.col(UTILITY_INDEX_COL_NAME).isin(valid_items)
#                 )
            
#             elif regex is not None:
#                 index_df = self.data.select(UTILITY_INDEX_COL_NAME).distinct()
#                 index_values = [row[UTILITY_INDEX_COL_NAME] for row in index_df.collect()]
                
#                 matched_indices = [
#                     idx for idx in index_values 
#                     if re.search(regex, str(idx))
#                 ]
...
#                 result_df = self.data.filter(
#                     F.col(UTILITY_INDEX_COL_NAME).isin(matched_indices)
#                 )
